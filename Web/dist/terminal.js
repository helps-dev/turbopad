...
(() => {
  ...
  function renderTable(visibleMarkets) {
    ...
    table.innerHTML = `<table class="market-table"><thead><tr><th>Market</th><th>Price</th><th>24h change</th><th>Volume</th><th>Live trend</th><th>Score</th><th><span class="sr-only">Details</span></th></tr></thead><tbody>${visibleMarkets.map((market, rank) => `<tr><td><div class="table-token"><span class="rank">${String(rank + 1).padStart(2, '0')}</span><div class="avatar" style="--avatar:${market.color}">${escapeHtml(market.symbol[0])}${market.icon ? `<img src="${escapeHtml(market.icon)}" alt="" loading="lazy" onerror="this.remove()">` : ''}</div><div><b>${escapeHtml(market.name)}</b><small>${escapeHtml(market.symbol)} <span>· ${escapeHtml(market.source)}</span></small></div></div></td><td>$${escapeHtml(market.price)}</td><td class="${market.change < 0 ? 'negative' : 'up'}">${market.change > 0 ? '+' : ''}${market.change.toFixed(2)}%</td><td>$${market.volume}</td><td><div class="table-spark">${window.TurboPad.sparkline(market, 96, 36)}</div></td><td><span class="table-score">${market.score ? `ϟ ${market.score}` : 'RWA'}</span></td><td><button data-detail="${escapeHtml(market.id)}" aria-label="View ${escapeHtml(market.name)}">↗</button></td></tr>`).join('')}</tbody></table>`;
  }
